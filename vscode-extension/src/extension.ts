...
async function updateApiKeyStatusBar(statusBarItem: vscode.StatusBarItem, secrets: vscode.SecretStorage) {
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (legacyKey) {
    ...
    await legacyConfig.update('apiKey', undefined, vscode.ConfigurationTarget.Global);
  }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('reposage.reviewCurrentFile', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showInformationMessage('Open a file to review it with RepoSage.');
        return;
      }
      ...
      vscode.window.showInformationMessage(`RepoSage: Reviewing ${fileName}...`);
      ...
      if (result.success) {
        ...
        vscode.window.showInformationMessage('RepoSage review complete! Check the sidebar for details.');
      } else {
        provider.setError(result.error || 'Unknown error');
        vscode.window.showErrorMessage(`RepoSage review failed: ${result.error}`);
      }
