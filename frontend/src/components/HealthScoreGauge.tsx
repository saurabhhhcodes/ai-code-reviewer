...
function computeHealthScore(fileReviews: Record<string, FileReview>): number {
  ...
  return Math.max(
    0,
    Math.round(100 - totalBugs * 3 - totalSecurityIssues * 15 - totalOptimizations * 1 - totalStylingIssues * 0.5),
  );
}

function getScoreColor(score: number): { text: string; bg: string; border: string; label: string } {
  if (score >= 80)
    return { text: '#22c55e', bg: 'rgba(34, 197, 94, 0.1)', border: 'rgba(34, 197, 94, 0.28)', label: 'Excellent' };
  if (score >= 60)
    return { text: '#3b82f6', bg: 'rgba(59, 130, 246, 0.1)', border: 'rgba(59, 130, 246, 0.28)', label: 'Good' };
  if (score >= 40)
    return { text: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)', border: 'rgba(245, 158, 11, 0.28)', label: 'Fair' };
          ...
          {isLoading ? (
            <div
              style={{
                ...
                height: '140px',
                borderRadius: '50%',
                background: 'var(--chart-track, rgba(255,255,255,0.03))',
                display: 'flex',
                alignItems: 'center',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            ...
            <div key={tier.range} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div
                style={{ width: '10px', height: '10px', borderRadius: '50%', background: tier.color, flexShrink: 0 }}
              />
              <span
                style={{ fontSize: '11px', color: 'var(--text-color, #f3f4f6)', fontWeight: 600, minWidth: '50px' }}
              >
                {tier.range}
              </span>
