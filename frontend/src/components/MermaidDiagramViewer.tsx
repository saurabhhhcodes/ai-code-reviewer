...
export default function MermaidDiagramViewer({ chart, repoName }: MermaidViewerProps) {
  ...
  const svgDataUrl = svg ? `data:image/svg+xml;charset=utf-8,${encodeURI(svg)}` : null;
  ...
  if (error) {
    return (
      ...
      >
        <div style={{ fontWeight: 700, marginBottom: '6px' }}>Mermaid Rendering Failed</div>
    ...
    >
      ...
      >
        {svgDataUrl ? (
          ...
          <span style={{ color: '#9ca3af', fontSize: '12px' }}>Generating visual flowchart...</span>
        )}
      </div>
    </div>
  );
