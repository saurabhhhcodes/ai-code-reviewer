...
const AnalyticsTrendsChart: React.FC = () => {
  ...
  const maxValue = Math.max(1, ...trends.flatMap((t) => SERIES.map((s) => t[s.key])));
  ...
  const buildPath = (key: (typeof SERIES)[number]['key']) =>
    trends.map((t, i) => `${i === 0 ? 'M' : 'L'} ${getX(i).toFixed(1)} ${getY(t[key]).toFixed(1)}`).join(' ');
    ...
    >
      ...
      <div style={{ width: '100%', boxSizing: 'border-box', overflow: 'hidden' }}>
        ...
        >
          ...
          {SERIES.map((s) =>
            trends.map((t, i) => (
              <circle key={`${s.key}-${i}`} cx={getX(i)} cy={getY(t[s.key])} r={2.5} fill={s.color}>
