...
test('IPv4: reports correct line number', () => {
  const content = ['// config file', 'const port = 3000;', 'const dbHost = "8.8.8.8";', 'module.exports = {};'].join(
    '\n',
  );
  const findings = scanSecrets(content);
...
test('scanSecrets: detects IPv4 and ETH wallet in same file', () => {
  const content = ['const server = "8.8.8.8";', 'const wallet = "' + ethAddress + '";'].join('\n');
  const findings = scanSecrets(content);
...
test('scanSecretsInChanges: detects IPv4 in PR diff changes', () => {
  const changes = [{ line: 5, content: 'const host = "8.8.8.8";' }];
  const results = scanSecretsInChanges(changes);
  assert.ok(results.findings.length >= 1, 'should detect IPv4 in changes');
  assert.ok(
    results.findings.some((f) => f.comment.includes('Hardcoded IPv4 Address')),
    'comment should mention IPv4',
  );
});

test('scanSecretsInChanges: detects ETH wallet in PR diff changes', () => {
  const changes = [{ line: 12, content: `const w = "${ethAddress}";` }];
  const results = scanSecretsInChanges(changes);
  assert.ok(results.findings.length >= 1, 'should detect ETH wallet in changes');
  assert.ok(
    results.findings.some((f) => f.comment.includes('Ethereum')),
    'comment should mention Ethereum',
  );
});

test('scanSecretsInChanges: detects BTC wallet in PR diff changes', () => {
  const changes = [{ line: 20, content: `const addr = "${btcBech32}";` }];
  const results = scanSecretsInChanges(changes);
  assert.ok(results.findings.length >= 1, 'should detect BTC wallet in changes');
  assert.ok(
    results.findings.some((f) => f.comment.includes('Bitcoin')),
    'comment should mention Bitcoin',
  );
});

test('scanSecretsInChanges: does NOT flag loopback IP in changes', () => {
  const changes = [{ line: 1, content: 'server.listen("127.0.0.1", 8080);' }];
  const results = scanSecretsInChanges(changes);
