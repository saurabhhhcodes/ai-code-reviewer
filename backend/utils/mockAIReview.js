      ...
      security: [
        {
          ...
          line: getRandomLine(),
          description:
            'Potential hardcoded credentials detected. API keys should always be loaded from environment variables (.env).',
      ...
      optimization: [
        {
          ...
          line: getRandomLine(),
          description:
            'Avoid using nested iterations if time complexity grows quadratically. Consider using a Map/Dictionary lookup.',
...
Generated automatically by **RepoSage AI Generator**.`;
  ...
  const mockMermaid = `graph TD\n  Root["📦 ${repoName}"]\n  ${files
    .slice(0, 5)
    .map((f, i) => `  Root --> File_${i}["📄 ${f.name.split('/').pop()}"]`)
    .join('\n')}`;
